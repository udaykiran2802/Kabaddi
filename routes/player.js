...
router.get('/:id/progress', async (req, res) => {
    ...
    // Determine which points to extract based on searchTerm
    const pointsKey = pointsMap[searchTerm] || 'ridePoints'; // Default to servePoints if searchTerm is invalid
    const servePointsArray = player.progress.map(progressEntry => progressEntry[pointsKey]);
    const labels = servePointsArray.map((_, index) => `${index + 1}`);
    console.log("--------------------------------")
    console.log(preData);
...
router.post('/:id', isLogggedin,isOwner,wrapAsync(async function (req, res) {
    let { id } = req.params;
    const { ridePoints, defensePoints } = req.body;
    ...
    searchTerm = 'ride'
    // Determine which points to extract based on searchTerm
    const pointsKey = 'ridePoints'; // Default to servePoints if searchTerm is invalid
    const servePointsArray = player.progress.map(progressEntry => progressEntry[pointsKey]);
    const labels = servePointsArray.map((_, index) => `${index + 1}`);
